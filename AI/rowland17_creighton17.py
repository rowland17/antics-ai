...
class AIPlayer(Player):
    ...
    def __init__(self, inputPlayerId):
        ...
        """
        self.state_list_file = 'test_list'
    ...
    #     # [if an attacker is leaving home area] 1
    #     # move_inputs.append(
    #     #     int(any([ant for ant in our_offense if ant.coords not in our_range])))
    ...
    #     # if len(self.last_states):
    #     #     curr_w_coords = [ant.coords for ant in our_workers]
    #     #     old_inv = utils.getCurrPlayerInventory(self.last_states[-1])
    #     #     old_workers = [
    #     #         ant for ant in old_inv.ants if ant.type == c.WORKER]
    #     #     old_w_coords = [ant.coords for ant in old_workers]
    #     #     if curr_w_coords == curr_w_coords:
    #     #         inputs.append(1)
    #     #     else:
    #     #         inputs.append(0)
    #     # else:
    ...
    def update_state(self, state, status='other'):
        ...
        # if cstate not in self.state_list:
            # print "new state! Current states: {}".format(len(self.state_list))

        # print "state reached: {}, current util: {}".format(cstate, self.state_list.get(cstate, 0))

        # next_cstate = cstate
        # for prev_state in reversed(self.last_states):
        #     prev_cstate = self.condense_state(prev_state)
        #     util = self.reward(status) + self.alpha * \
        #         (0 + self.discount * self.state_list[next_cstate])
        #     self.state_list[prev_state] = util
        #     # win/loss only counts on first state
        #     status = 'other'
        #     next_cstate = prev_cstate
